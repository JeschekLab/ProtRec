...
p <- ggplot() +
  ...
  theme_SH() +
  scale_x_continuous('randomised position') +
...
p <- ggplot() +
  ...
  theme_SH() +
  scale_x_continuous('randomised position in TEVs',
    breaks = seq(1, 7, 1)) +
...
w <- df %>%
  group_by(BC_TEVs) %>%
  summarize(reads_BC_TEVs = n()) %>%
  ungroup() %>%
  arrange(desc(reads_BC_TEVs))

x <- df %>%
  group_by(BC_TEVs, TEVs) %>%
  summarize(reads_TEVsxBC = n()) %>%
  ungroup() %>%
