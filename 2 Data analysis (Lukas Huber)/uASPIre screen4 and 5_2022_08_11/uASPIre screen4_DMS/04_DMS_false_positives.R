...
dir.create("./04_DMS_false_positives")
dir.create("./04_DMS_false_positives/results")
dir.create("./04_DMS_false_positives/plots")


# prepare data ------------------------------------------------------------

# read BC data after removing the mutated backbones done earlier
data_BCs <- read.table("./01.1_DMS_mutated_backbones/results/Data_DMS_unmapped_without_mutated_backbones.txt", sep = "\t", header = TRUE)

## recalculate the AUCs after removing the mutated backbones ----
...
data <- inner_join(data,
                   data_BCs %>% select(TEVp, TEVs, n_mut_TEVp, n_mut_TEVs) %>% distinct())
...
# # Print the result
# cat("Fraction of unique TEVp-TEVs combinations lost when applying a min 20 read threshold per time point on DNA rather than protein level:",
#     ratio, "%")
...
#save plot
save_plot(p, filename = "./04_DMS_false_positives/plots/histogram_replicates_before_removing_outliers_min20protein.pdf",
          base_height = 3.71, 
          base_asp = 1)
...
#save plot
save_plot(p, filename = "./04_DMS_false_positives/plots/histogram_replicates_before_removing_outliers_min20DNA.pdf",
          base_height = 3.71, 
          base_asp = 1)
...
noise <- read.table("./03_DMS_controls/results/df_noise.txt", sep = "\t", header = TRUE)
...
data_min20DNA <- inner_join(data_min20DNA,
                            data_BCs_min20DNA %>% select(TEVp, TEVs, n_mut_TEVp, n_mut_TEVs) %>% distinct())
...
for (alternative in c(1:2)) {
  ...
  dir.create(paste0("./04_DMS_false_positives/plots/", alternatives[alternative]))
  ...
  write.table(data.frame(n = num_unique_combinations,
                         criteria = message),
              file = paste0("./04_DMS_false_positives/results/n_biol_reps_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  ...
  # write.table(compareAUCs, file = paste0("./04_DMS_false_positives/results/compare_AUCs_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  ...
  # save result to file
  write.table(df_false_freq, file = paste0("./04_DMS_false_positives/results/df_false_freq_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  ...
  print(p)
  # save plot
  save_plot(p, filename = paste0("./04_DMS_false_positives/plots/", alternatives[alternative], "/false_positives_density.pdf"),
            base_height = 3,
            base_asp = 2)
  
  # create histogram plot
  p <- ggplot(compareAUCs, aes(x = fc, fill = "#8da0cb")) +
    geom_histogram(position = "identity", fill = "#8da0cb", alpha = 0.7, color = "black") +
    annotate("text", x = max(compareAUCs$fc)/2, y = 20,
             label = paste("n =", num_unique_combinations)) +
    theme_cowplot(font_size = 12) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
    theme(legend.position = "none") + # Remove the legend
  ...
  print(p)
  # save plot
  save_plot(p, filename = paste0("./04_DMS_false_positives/plots/", alternatives[alternative], "/false_positives_histogram.pdf"),
            base_height = 3,
            base_asp = 1.7)
...
for (alternative in c(1:2)) {
  ...
  for (i in c(1:5)) {
    ...
    p_TEVs <- ggplot(p_TEVs_data, aes(x = as.integer(timepoint), y = fraction_flipped, col = color)) +
      ...
      geom_point(data = filter(p_TEVs_data, TEVs == hitTEVs), aes(group = TEVs), color = "#ca0020") +
      geom_line(data = filter(p_TEVs_data, TEVs == hitTEVs), aes(group = TEVs), color = "#ca0020")
    
    # If "ENLYFQS" is present in the data, also plot this curve again to ensure it is in the front
    if ("ENLYFQS" %in% p_TEVs_data$TEVs) {
      # If present, add the layers for "ENLYFQS"
      p_TEVs <- p_TEVs +
        geom_point(data = filter(p_TEVs_data, TEVs == "ENLYFQS"), aes(group = TEVs), color = "#0571b0") +
        geom_line(data = filter(p_TEVs_data, TEVs == "ENLYFQS"), aes(group = TEVs), color = "#0571b0")
    }
    ...
    print(pg)
    # and save
    save_plot(pg, filename = paste0("./04_DMS_false_positives/plots/", alternatives[alternative], "/fc_top", i, "_", hitTEVp, "_", hitTEVs, ".pdf"),
              base_height = 3,
              base_asp = 2.7)
  }
}
...
for (alternative in c(1:2)) {
  ...
  # add important columns (n_mut_TEVp, n_mut_TEVs)
  ...
  # # write table to file
  # write.table(over3BCs_fc, file = paste0("./04_DMS_false_positives/results/replicates_cleaned_by_fc_", alternatives[[alternative]],".txt"), 
...
for (alternative in c(1:2)) {
  ...
  write.table(data_cleaned_by_fc, file = paste0("./04_DMS_false_positives/results/Data_DMS_fc_corrected_", alternatives[[alternative]],".txt"), 
              sep = "\t", row.names=FALSE, quote = F)
...
for (alternative in c(1:2)) {
  ...
  #save plot
  save_plot(p, filename = paste0("./04_DMS_false_positives/plots/histogram_replicates_fc_corrected_", alternatives[[alternative]], ".pdf"),
            base_height = 3.71, 
            base_asp = 1)
}
...
for (alternative in c(1:2)) {
  ...
  p <- ggplot(TEVp_BCs_counts, aes(x = n)) +
    geom_histogram(binwidth = 1, fill = "#49848a", color = "white", alpha = 1) +
    # scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
    # geom_text(stat = 'count', aes(label = ..count..), size = 3, angle = 90, vjust = 0.5, hjust = -0.2) +
  ...
  print(p)
  #save plot
  save_plot(p, filename = paste0("./04_DMS_false_positives/plots/histogram_TEVp_BCs_fc_corrected_", alternatives[[alternative]], ".pdf"),
            base_height = 3.71, 
            base_asp = 0.8)
  
  # make a histogram plot of TEVs BC counts
  p <- ggplot(TEVs_BCs_counts, aes(x = n)) +
    geom_histogram(binwidth = 1, fill = "#bc9669", color = "white", alpha = 1) +
  ...
  print(p)
  #save plot
  save_plot(p, filename = paste0("./04_DMS_false_positives/plots/histogram_TEVs_BCs_fc_corrected_", alternatives[[alternative]], ".pdf"),
            base_height = 3.71, 
            base_asp = 0.8)
}
