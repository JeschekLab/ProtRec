...
# define technical replicate ---------------------------------------------------

# define the technical replicate (i.e. shake flask) this analysis should be done for
flask <- 1
# define HOME directory
HOME <- paste0("./03_false_positives_replicate_", flask)

# create folders ----------------------------------------------------------

# create a folder to store the individual plots and results in
dir.create(HOME)
dir.create(paste0(HOME, "/results"))
dir.create(paste0(HOME, "/plots"))


# prepare data ------------------------------------------------------------

# load data
data <- read.table("./data/02_LH_9k_NNK_final.txt", sep = "\t", header = TRUE) %>%
  # all this analysis is done with replicate 1 only!
  filter(rep == flask) %>%
  # throw out unnecessary columns
  select(-rep, -AUC1, -AUC2, -AUC3) %>%
  # rename AUC4 to AUC
  rename(AUC = AUC4) %>%
  # add a column specifying the AA in P1'
  mutate(TEVs_AA = substring(TEVs, 7, 7)) 
data_BCs <- read.table("./data/00_orthogonal_check_all.txt", sep = "\t", header = TRUE) %>%
  # rename a mislabeled column
  rename(reads_BCTEVpxTEVp = reads_BCTEVsxBCTEVp) %>%
  # rename another column to stay consistent
  rename(TEVs_AA = P1p, AUC = AUC4)

# load data of controls
internal_controls <- read.table("./01_internal_controls/results/internal_controls.txt", sep = "\t", header = TRUE, quote = "")

# Can we afford a min20 threshold on DNA level? ----

# count how many unique TEVp-TEVs combinations we covered
...
# --> NO, we can't afford a minimal read threshold of 20 on DNA level! Instead,
# we sum up synonymous variants and set the threshold on protein level.
...
p <- ggplot(replicate_samples, aes(x = n_replicates)) +
  geom_histogram(binwidth = 1, fill = "#69498a", color = "black", alpha = 0.7) +
...
#save plot
save_plot(p, filename = paste0(HOME, "/plots/histogram_replicates_before_removing_outliers_min20protein.pdf"),
          base_height = 3.71, 
          base_asp = 1)
...
#save plot
save_plot(p, filename = paste0(HOME, "/plots/histogram_replicates_before_removing_outliers_min20DNA.pdf"),
          base_height = 3.71, 
          base_asp = 1)
...
# manually list a few variants to be investigated, i.e. variants that could not
# be validated 
suspicious <- c('HGR', 'HPG', 'IGR', 'RAV', 'VDS')

for (variant in suspicious) {
  # generate plot
  p_TEVs <- data %>%
    # filtering
    filter(min >= 20,
    ...
    select(TEVp, TEVs_AA, t1, t2, t3, t4, t5) %>%
    # rename for correct plotting
    rename("0" = "t1") %>% rename("1" = "t2") %>% rename("2" = "t3") %>%
    rename("3" = "t4") %>% rename("4" = "t5") %>%
    # reformatting for plotting
    pivot_longer(cols = !c(TEVp, TEVs_AA) , names_to = "timepoint", values_to = "fraction_flipped") %>%
    # plotting
    ggplot(., aes(x = as.integer(timepoint), y = fraction_flipped, col = TEVs_AA)) +
    geom_point() +
    geom_line() +
    theme_cowplot(font_size = 12) +
    ...
    labs(col = "AA in P1'") +
    ggtitle(paste("TEVp I", variant)) +
    theme(plot.title = element_text(hjust = 0.5))

  # extract the P1' AA with the highest AUC, i.e., the problematic P1'
  hitP1p <- data %>%
    filter(TEVp == variant) %>%
    arrange(desc(AUC)) %>%
    pull(TEVs_AA) %>% .[1]
  ...
  ### go to the orthogonal check data set and extract this hit with its problematic P1'

  # First, calculate the fractions flipped for all BCs combined to add it to the plot later
  BCs_combined <- data_BCs %>%
    filter(TEVp == variant, TEVs_AA == hitP1p) %>%
    group_by(TEVp) %>%
    ...
    select(BC_TEVp, t1, t2, t3, t4, t5, sum, style)

  # prepare data for plotting
  hit <- data_BCs %>%
    # filtering
    filter(TEVp == variant, TEVs_AA == hitP1p) %>%
    mutate(style = '1') %>% # add a style for plotting
    # selecting only necessary columns
    select(BC_TEVp, t1, t2, t3, t4, t5, sum, style) %>%
  ...
  p_TEVp <- ggplot(hit, aes(x = as.integer(timepoint), y = fraction_flipped, color = sum, group = BC_TEVp, linetype = style)) +
    theme_cowplot(font_size = 12) +
    geom_point() +
    geom_line() +
    scale_x_continuous("Time after induction (h)", limits = c(0, 4)) +
    ...
    theme(legend.text = element_text(size=10)) + # change size of legend text
    ggtitle(paste0("TEVp I ", variant, "\nAA in P1': ", hitP1p)) + # add plot title
    theme(plot.title = element_text(hjust = 0.5))
  ...
  print(pg)
  # and save
  save_plot(pg, filename = paste0(HOME, "/plots/False_positive_", variant, ".pdf"),
            base_asp = 2)
}
...
cutoff <- noise %>% filter(outlier_removed == FALSE) %>% pull(cutoff)
...
for (alternative in c(1:2)) {
  ...
  dir.create(paste0(HOME, "/plots/", alternatives[alternative]))
  ...
  write.table(data.frame(n = num_unique_combinations,
                         criteria = message),
              file = paste0(HOME, "/results/n_biol_reps_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  ...
  compareAUCs <- filter(compareAUCs, min_wo_max_over20 == TRUE) %>%
    # add a TEVs_AA column for plotting later
    mutate(TEVs_AA = gsub('ENLYFQ', '', TEVs))
  
  # save this table to a list
  list_compareAUCs[[alternative]] <- compareAUCs
  
  # save this table to a file
  write.table(compareAUCs, file = paste0(HOME, "/results/compare_AUCs_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  ...
  # save result to file
  write.table(df_false_freq, file = paste0(HOME, "/results/df_false_freq_", alternatives[alternative], ".txt"), sep = "\t", row.names=FALSE, quote = F)
  
  # create a density plot colored by TEVs
  p <- ggplot(compareAUCs, aes(x = fc, fill = TEVs_AA)) +
  ...
  print(p)
  # save plot
  save_plot(p, filename = paste0(HOME, "/plots/", alternatives[alternative], "/false_positives_density.pdf"),
            base_height = 3,
            base_asp = 2)
  
  # create histogram plot
  p <- ggplot(compareAUCs, aes(x = fc, fill = "#69498a")) +
    geom_histogram(position = "identity", fill = "#69498a", alpha = 0.7, color = "black") +
  ...
  # scale_y_log10()  # Transform x-axis to a log scale
  
  # show plot
  print(p)
  # save plot
  save_plot(p, filename = paste0(HOME, "/plots/", alternatives[alternative], "/false_positives_histogram.pdf"))
...
for (alternative in c(1:2)) {
  ...
  for (i in c(1:5)) {
    ...
    print(pg)
    # and save
    save_plot(pg, filename = paste0(HOME, "/plots/", alternatives[alternative], "/fc_top", i, "_", hitTEVp, "_", hitTEVs, ".pdf"),
              base_height = 3,
              base_asp = 2.7)
  }
}
...
for (alternative in c(1:2)) {
  ...
  # # write table to file
  # write.table(over3BCs_fc, file = paste0(HOME, "/results/replicates_cleaned_by_fc_", alternatives[[alternative]],".txt"), 
...
for (alternative in c(1:2)) {
  ...
  write.table(data_cleaned_by_fc, file = paste0(HOME, "/results/Data_ABC_fc_corrected_", alternatives[[alternative]],".txt"), 
              sep = "\t", row.names=FALSE, quote = F)
...
for (alternative in c(1:2)) {
  ...
  # make a histogram plot of replicates
  p <- ggplot(replicate_samples, aes(x = n_replicates)) +
    geom_histogram(binwidth = 1, fill = "#69498a", color = "white", alpha = 0.7) +
  ...
  #save plot
  save_plot(p, filename = paste0(HOME, "/plots/histogram_replicates_fc_corrected_", alternatives[[alternative]], ".pdf"),
            base_height = 3.6, 
            base_asp = 0.7)
}
...
for (alternative in c(1:2)) {
  ...
  # make a histogram plot of TEVp BC counts
  p <- ggplot(TEVp_BCs_counts, aes(x = n)) +
    geom_histogram(binwidth = 0.05, fill = "#49848a", color = "white", alpha = 1) +
    # scale_y_continuous(expand = expansion(mult = c(0, 0.2))) +
    # geom_text(stat = 'count', aes(label = ..count..), size = 3, angle = 90, vjust = 0.5, hjust = -0.2) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
    scale_x_continuous(trans = 'log10') +
    theme_cowplot(font_size = 12) +
    labs(x = "Number of TEVp barcodes", y = "Count of TEVp variants") +
    annotate("text", x = Inf, y = Inf, hjust = 1, vjust = 1,
             label = paste("Average no. of BCs \nper TEVp =", round(sum(TEVp_BCs_counts$n)/length(TEVp_BCs_counts$TEVp), 1))) 
  # print plot  
  print(p)
  #save plot
  save_plot(p, filename = paste0(HOME, "/plots/histogram_TEVp_BCs_fc_corrected_", alternatives[[alternative]], ".pdf"),
            base_height = 3.6, 
            base_asp = 0.7)
